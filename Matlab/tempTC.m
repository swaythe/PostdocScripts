...
%     tc = squeeze(nanmean(tc,3));
    timecourse = tc;
    tc = zeros(20,size(timecourse,2),size(timecourse,3),size(timecourse,4),size(timecourse,5));
    
    % Now fold some rows into each other so we have 20 unique Coh-Dir pairs
    % rather than 64
    % Coh = 0, Dir = All
    tc(1,:,:,:,:) = nanmean(timecourse([29 61],:,:,:,:));
    tc(2,:,:,:,:) = nanmean(timecourse([1 25 33 57],:,:,:,:));
    tc(3,:,:,:,:) = nanmean(timecourse([5 21 37 53],:,:,:,:));
    tc(4,:,:,:,:) = nanmean(timecourse([9 17 41 49],:,:,:,:));
    tc(5,:,:,:,:) = nanmean(timecourse([13 45],:,:,:,:));
    
    % Coh = 12, Dir = All
    tc(6,:,:,:,:) = nanmean(timecourse([30 62],:,:,:,:));
    tc(7,:,:,:,:) = nanmean(timecourse([2 26 34 58],:,:,:,:));
    tc(8,:,:,:,:) = nanmean(timecourse([6 22 38 54],:,:,:,:));
    tc(9,:,:,:,:) = nanmean(timecourse([10 18 42 50],:,:,:,:));
    tc(10,:,:,:,:) = nanmean(timecourse([14 46],:,:,:,:));
    
    % Coh = 25, Dir = All
    tc(11,:,:,:,:) = nanmean(timecourse([31 63],:,:,:,:));
    tc(12,:,:,:,:) = nanmean(timecourse([3 27 35 59],:,:,:,:));
    tc(13,:,:,:,:) = nanmean(timecourse([7 23 39 55],:,:,:,:));
    tc(14,:,:,:,:) = nanmean(timecourse([11 19 43 51],:,:,:,:));
    tc(15,:,:,:,:) = nanmean(timecourse([15 47],:,:,:,:));
    
    % Coh = 100, Dir = All
    tc(16,:,:,:,:) = nanmean(timecourse([32 64],:,:,:,:));
    tc(17,:,:,:,:) = nanmean(timecourse([4 28 36 60],:,:,:,:));
    tc(18,:,:,:,:) = nanmean(timecourse([8 24 40 56],:,:,:,:));
    tc(19,:,:,:,:) = nanmean(timecourse([12 20 44 52],:,:,:,:));
