...
%  end
 ...
 cd('/home/sshankar/Categorization/Scripts/')
%  analyzeCategory;
 dirs = [0 15 45 75 90];
 %
 % Panel a
...
%  offset = 0;
%  for pi = 1:5
%      plot(xx1+offset,pc_cond(:,pi), 'o', 'color', cs(pi,:), 'MarkerSize',msize2,'Linewidth', line2,'LineStyle','-')
%      offset = offset + 2.25;
...
%  offset = 0;
%  for pi = 1:5
%      plot(xx1+offset,rtm_cond(:,pi),'o','color',cs(pi,:),'MarkerSize',msize2,'Linewidth', line2,'LineStyle','-');
%      for rti = 1:length(rtm_cond(:,pi))
%          line([xx1(rti)+offset xx1(rti)+offset],[rtm_cond(rti,pi)+rtsd_cond(rti,pi) rtm_cond(rti,pi)-rtsd_cond(rti,pi)],'color',cs(pi,:),'MarkerSize',msize2,'Linewidth', line2,'LineStyle','-');
%      end
%      offset = offset + 2.25;
...
%      text(col1+100,pc_cond(5,pi),[num2str(dirs(pi)) '%'],'Fontsize',fnt0,'HorizontalAlignment','Left');
 ...
 print -depsc '/home/sshankar/Categorization/Figures/ScanSub02_coh.eps'
