...
for si = 1:length(subs)
    ...
    for ri = 1:length(ROI)
        ...
        temp = robustfit(cat(1,rts{4,:}),cat(1,betas{4,:,ROI(ri)}));
        slopes(si,ri) = temp(2);
        ints(si,ri) = temp(1);
        plot(cat(1,rts{4,:}),cat(1,betas{4,:,ROI(ri)}),'k.')
        plot(cat(1,rts{4,:}),temp(1)+temp(2)*cat(1,rts{4,:}),'r-')
        
        temp = robustfit(cat(1,rtc{4,:}),cat(1,betac{4,:,ROI(ri)}));
        slopec(si,ri) = temp(2);
        intc(si,ri) = temp(1);
        
        temp = robustfit(cat(1,rti{4,:}),cat(1,betai{4,:,ROI(ri)}));
        slopei(si,ri) = temp(2);
        inti(si,ri) = temp(1);
    end
end
