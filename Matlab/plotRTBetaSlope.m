...
nROI = length(ROI);
nCoh = size(RTBSlopeCD,1);
nDist = size(RTBSlopeCD,2);
  
maxR = ones(1,nROI);
minR = zeros(1,nROI);
for ri = 1:nROI
    minR(ri) = min(min(min(squeeze(RTBSlopeCD(:,:,ri,:)))));
    maxR(ri) = max(max(max(squeeze(RTBSlopeCD(:,:,ri,:)))));
    maxR(maxR==0) = 1;
end
...
%                 plot(RTBSlopeCD(ci,di,ri,si), symb{si})
...
for si = 1:length(subs)
    for ri = 1:nROI
        ...
        for ci = 1:nCoh
            ...
            plot(RTBSlopeC(ci,ri,si), symb{si})
            if si==9
                if ci==2
                    title(roi_name{ri})
                elseif ci==1
            ...
            plot(RTcBSlopeC(ci,ri,si), symb{si})
            if si==9 && ci==1
                ylabel('Correct')
            end
            axis([0 2 minR(ri) maxR(ri)])
            subplot(3,nCoh,ci+nCoh*2); hold on
            plot(RTiBSlopeC(ci,ri,si), symb{si})
            if si==9 && ci==1
                ylabel('Incorrect')
            end
            axis([0 2 minR(ri) maxR(ri)])
...
%             plot(RTBSlopeD(di,ri,si), symb{si})
%             if si==9
%                 if di==3
%                     title(roi_name{ri})
%                 elseif di==1
...
%             plot(RTcBSlopeD(di,ri,si), symb{si})
%             if si==9 && di==1
%                 ylabel('Correct')
%             end
%             axis([0 2 minR(ri) maxR(ri)])
%             subplot(3,nDist,di+nDist*2); hold on
%             plot(RTiBSlopeD(di,ri,si), symb{si})
%             if si==9 && di==1
%                 ylabel('Incorrect')
%             end
%             axis([0 2 minR(ri) maxR(ri)])
